...
const ERROR_MESSAGES: Record<string, string> = {
  PRE_REGISTRATION_EMAIL_EXISTS: "이미 신청된 이메일입니다.",
...
export function CTASection() {
  ...
  const mutation = usePreRegistration({
    onSuccess: () => {
      ...
      setFeedback({
        type: "success",
        message: "신청 완료! 입력하신 이메일로 안내 메일을 보냈어요.",
      });
    },
    onError: (error) => {
      setFeedback({
            ...
            >
              <div className="flex items-center gap-3 max-md:flex-col">
                <input
                  ...
                  className="flex-1 h-15 px-7.5 py-4 rounded-full bg-neutral-100/15 border border-neutral-300 text-body-l text-neutral-300 placeholder:text-neutral-300 text-center outline-none focus:border-neutral-50 transition-colors disabled:opacity-60 max-md:w-full"
                />
                <Button
                  type="submit"
                  hierarchy="secondary"
